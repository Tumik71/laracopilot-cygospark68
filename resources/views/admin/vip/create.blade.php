...
@section('title', 'Přidat VIP obsah')
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.vip.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Zpět</a>
    <h1 class="text-2xl font-bold text-gray-800 mt-1">👑 Přidat VIP obsah</h1>
</div>
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow p-6">
        <form action="{{ route('admin.vip.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
            ...
            <div>
                ...
                <select name="type" required class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">
                    ...
                    <option value="guide">📋 Průvodce</option>
                    <option value="download">⬇️ Ke stažení</option>
                    <option value="webinar">🎥 Webinář</option>
                </select>
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Obsah / Popis *</label>
                <textarea name="content" rows="8" required class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">{{ old('content') }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">YouTube URL (volitelné)</label>
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Příloha ke stažení (volitelné, max 50 MB)</label>
                <input type="file" name="file" class="w-full border border-gray-300 rounded-lg px-3 py-2">
            </div>
            <div class="flex justify-end space-x-3">
                ...
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-yellow-400 to-red-500 text-white font-semibold rounded-lg hover:opacity-90 transition">Uložit VIP obsah</button>
            </div>
        </form>
    </div>
</div>
