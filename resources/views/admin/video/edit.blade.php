...
@section('title', 'Upravit video')
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.video.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Zpět</a>
    <h1 class="text-2xl font-bold text-gray-800 mt-1">✏️ Upravit video</h1>
</div>
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow p-6">
        <form action="{{ route('admin.video.update', $video->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" required value="{{ old('title', $video->title) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Popis</label>
                <textarea name="description" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">{{ old('description', $video->description) }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">YouTube URL</label>
                <input type="url" name="youtube_url" value="{{ old('youtube_url', $video->youtube_url) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">
            </div>
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Délka</label>
                    <input type="text" name="duration" value="{{ old('duration', $video->duration) }}" placeholder="12:34" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">
                </div>
            </div>
            <div>
            ...
            <div class="flex items-center">
                <input type="checkbox" name="vip_only" id="vip_only" value="1" {{ $video->vip_only ? 'checked' : '' }} class="mr-2">
